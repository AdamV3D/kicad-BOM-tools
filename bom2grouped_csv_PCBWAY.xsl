<!--XSL style sheet to convert EESCHEMA XML Partlist Format to grouped CSV BOM Format
	...
	Functionality:
		Generation of PCBway PCBA compatible BOM
    ...
    The command line is
        xsltproc -o "%O-PCBway.csv" "FullPathToFile/bom2grouped_csv_PCBway.xsl" "%I"
-->
<!--
    @package
    Generates a PCBway PCBA service compatible BOM
    ...
    * Components are sorted by ref and grouped by same value+footprint
    One value per line
    Fields are
	Designator,Qty,Mfg Part #(ManfNum), Value, Package/Footprint(abv.Footprint)
	Items in parenthesis are custom fields.
    
    The command line is
        xsltproc -o "%O-PCBway.csv" "full_path/bom2grouped_csv_PCBway.xsl" "%I"
-->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/export">
	    <xsl:text>Designator,Qty,Mfg Part #,Value,Package/Footprint</xsl:text>
	    <!-- all table entries -->
	...
	<xsl:template match="components">
	    ...
	    <xsl:for-each select="comp[count(. | key('partTypeByValueAndFootprint', concat(footprint, '-', value))[1]) = 1]">
			...
			<xsl:value-of select="fields/field[@name='ManfNum']"/><xsl:text>","</xsl:text>
			<xsl:value-of select="value"/><xsl:text>","</xsl:text>
			<xsl:value-of select="fields/field[@name='abv.Footprint']"/><xsl:text>"</xsl:text>
